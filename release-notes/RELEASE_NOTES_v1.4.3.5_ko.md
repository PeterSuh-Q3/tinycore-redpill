...
로더가 인터넷 연결을 세 번 시도한 뒤에도 연결하지 못하면 고정 IP를 설정할지 묻습니다. `Yes`를 선택하면 저장소 복제나 모델 다운로드 또는 로더 빌드 없이 정적 네트워크 설정 화면으로 바로 진입합니다. 입력값은 `/mnt/tcrp/user_config.json`에 직접 저장된 뒤 실행 중인 FRIEND 커널에 즉시 적용되고 `menu.sh`는 정상 메인 메뉴로 돌아갑니다.
