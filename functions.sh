...
function reconcile_addon_flags() {
    ...
    [ -f "${bundled}" ] || return 0
    for addon in vmtools nvmesystem sanmanager-repair; do
...
function migrate_ipsettings_schema() {
    ...
    if [ "${t}" = "object" ]; then
        ...
        write_user_config_json "${cfg}" "${json}" || return 1
    elif [ "${t}" != "array" ]; then
        json=$(jq '.ipsettings = []' "${cfg}") || return 1
        write_user_config_json "${cfg}" "${json}" || return 1
    fi
    ...
    ' "${cfg}") || return 1
    write_user_config_json "${cfg}" "${json}"
}
...
# 2026.09.20 v1.4.4.5
# Responsive Alpine desktop reliability and optional SAN Manager repair
...
NCEOF
    ...
    # Intel GPU Top and AMDGPU Top are bundled and managed by MSHELL Manager,
    # so neither SPK is staged into /addons or installed by aeudev. If a
    # supported AMD display controller is present, stage only the AMD runtime.
    if [ ! -x "/home/tc/tools/install-amdgpu-addon.sh" ]; then
      ...
      if detect_amd_gpu && [ ! -s "${RAMDISK_PATH}/addons/amdgpu-driver.json" ]; then
        echo "[amdgpu] AMD GPU detected, but the verified AMD runtime was not embedded"
      fi
    fi
...
menuentry 'Alpine Redpill Image Build' {
        ...
        set gfxpayload=keep
        echo Loading Linux...
        linux /vmlinuz-lts loglevel=3 console=ttyS0 console=tty1 video=Virtual-1:1280x960 modules=nvme,nvme-core,hwmon kexec_load_disabled=0
        echo Loading initramfs...
