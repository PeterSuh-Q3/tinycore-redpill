...
EOF
    ...
    if [ "$RD_COMPRESSED" = "false" ]; then
        if [ "$FRKRNL" = "NO" ]; then
            if [ "${MDLNAME}" == "custom-modules" ]; then
                echo "Ramdisk in not compressed, use bsdcpio + xz -6 --format=lzma"
                (cd $rdtemp && sudo find . | sudo bsdcpio -o -H newc -R root:root | xz -6 --format=lzma > /mnt/${loaderdisk}3/initrd-dsm) >/dev/null
            else
