...
EOF
    ...
    if [ "$RD_COMPRESSED" = "false" ]; then
        if [ "$FRKRNL" = "NO" ]; then
            if [ "${MDLNAME}" == "custom-modules" ]; then
                echo "Ramdisk in not compressed, use bsdcpio + xz -9 -T0 --check=crc32"
                (cd $rdtemp && sudo find . | sudo bsdcpio -o -H newc -R root:root | xz -9 -T0 --check=crc32 > /mnt/${loaderdisk}3/initrd-dsm) >/dev/null
            else
