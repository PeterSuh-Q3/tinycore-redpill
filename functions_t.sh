...
function getredpillko() {
    ...
    if [ "${offline}" = "NO" ]; then
        ...
        RR-VER=""
        RR-VER="26.3.1"
        STATUS=`sudo curl --connect-timeout 5 -skL -w "%{http_code}" "https://github.com/PeterSuh-Q3/redpill-lkm${v}/releases/download/${TAG}/rp-lkms-${RR-VER}.zip" -o "/mnt/${tcrppart}/rp-lkms${v}.zip"`
    else
