    ...
    if [[ $BIOS_CNT -eq 1 ]] && [ "$FRKRNL" = "YES" ]; then 
        ...
        if [ "${BUS}" != "block" ]; then
            ...
            if [ -f ${patfile} ]; then
                ...
                if [ "$FRKRNL" = "NO" ]; then
                    dd if=${patfile} of=${local_cache}/$(basename ${patfile}) conv=fsync status=progress && rm -vf /home/tc/redpill-load/cache/*.pat                
                else
                    sudo dd if=${patfile} of=${local_cache}/$(basename ${patfile}) conv=fsync status=progress && sudo rm -vf /home/tc/redpill-load/cache/*.pat
                fi
            fi
