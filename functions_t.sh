...
function wr_part1() {
    ...
    if [ 0${TOTALUSED} -ge 0${SPACELEFT} ]; then
        ...
        if [ 0${TOTALUSED} -ge 0${SPACELEFT} ]; then
            [ mountpoint "${mdiskpart}" ] && sudo umount "${mdiskpart}"
...
if [ "${answer}" = "Y" ] || [ "${answer}" = "y" ]; then
    ...
    [ mountpoint "${synop1}" ] && sudo umount ${synop1} 
    [ mountpoint "${synop2}" ] && sudo umount ${synop2} 
    [ mountpoint "${synop3}" ] && sudo umount ${synop3}
