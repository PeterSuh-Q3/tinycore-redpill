...
# 이후 모든 코드가 항상 배열이라고 가정할 수 있게 한다. proxy는 NIC 개념이
# 아니므로 최상위 .netproxy.ipproxy로 옮긴다. 몇 번을 호출해도 안전하도록
# 매 단계 멱등이며, tcrpfriend의 boot.sh에도 동일한 로직이 별도 구현되어
# 있다(별개의 buildroot rootfs라 이 파일을 source할 수 없음).
function migrate_ipsettings_schema() {
    local cfg="$1" t json oldproxy
    [ -f "${cfg}" ] || return 0
    ...
    # 항목은 있는데 primary가 하나도 없으면(과거 배열 스키마 실험판 등) 첫
    # 항목을 primary로 승격한다 - "게이트웨이는 항상 정확히 1개" 불변식을
    # 이 함수를 거치기만 하면 항상 보장하기 위한 안전망.
...
# so multiple static NICs never fight over "ip route add default".
